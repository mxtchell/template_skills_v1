...
# MAIN SKILL
@skill(
    name="Sixt Performance Overview Drivers",
    llm_name="sixt_performance_overview_drivers",
    description="Analyzes DDR performance drivers with integrated trend analysis. Shows variance, percentage differences, and performance indicators across dimensions with supporting metrics trends for Sixt car rental data.",
    capabilities="Specialized for DDR1 and DDR2 vs target analysis with trend insights. Provides integrated analysis of performance drivers and supporting metrics without redundant queries. Shows monthly trends alongside aggregated performance.",
    ...
    example_questions="What's driving DDR1 performance vs target with trend analysis? Show me DDR2 performance with supporting metrics trends. Analyze damage detection variance with employee and digital adoption trends.",
    parameter_guidance="Select DDR1 or DDR2 for analysis. Specify time periods and optional breakout dimensions. Provides integrated trend analysis automatically.",
    parameters=[
        SkillParameter(
            name="periods",
...
def sixt_performance_overview_drivers(parameters: SkillInput):
    ...
    # Don't create supporting metrics yet - wait until we have trend data
    ...
    if check_vs_enabled([env.metric]):
        ...
        if isinstance(trend_result, tuple) and len(trend_result) == 2:
            ...
            if trend_metrics_df is not None:
                ...
                # Now create supporting metrics using the trend DataFrame
                try:
                    supporting_metrics_df = create_supporting_metrics_analysis(env, trend_metrics_df)
...
def create_supporting_metrics_analysis(env, trend_df=None):
    """Create supporting metrics analysis DataFrame for DDR root cause insights"""
    try:
        # If we have trend DataFrame, use it instead of running SQL
        if trend_df is not None and not trend_df.empty:
            # Aggregate the trend data to get summary metrics
            summary_data = {
                'supporting_metrics_summary': {
                    'actual_ddr1': env.da._metric_df.loc['ddr1', 'curr'] if 'ddr1' in env.da._metric_df.index else 0,
                    'target_ddr1': env.da._metric_df.loc['ddr1', 'prev'] if 'ddr1' in env.da._metric_df.index else 0,
                    'gap_vs_target': env.da._metric_df.loc['ddr1', 'diff'] if 'ddr1' in env.da._metric_df.index else 0,
                    'avg_checkin_volume': trend_df['checkin_count'].mean() if 'checkin_count' in trend_df.columns else 0,
                    'damage_detection_rate': trend_df['damage_at_check_in'].mean() if 'damage_at_check_in' in trend_df.columns else 0,
                    'avg_employee_months': trend_df['months_maturity_employee'].mean() if 'months_maturity_employee' in trend_df.columns else 0,
                    'live_check_in_rate': trend_df['live_check_in_rate'].mean() if 'live_check_in_rate' in trend_df.columns else 0
                }
            }
            
            return pd.DataFrame([summary_data])
            
        # Fallback to SQL query if no trend data available
...
if __name__ == '__main__':
    skill_input: SkillInput = sixt_performance_overview_drivers.create_input(
        arguments={
            "breakouts": [
                "brnc_name"
            ],
    ...
    out = sixt_performance_overview_drivers(skill_input)
    preview_skill(sixt_performance_overview_drivers, out)